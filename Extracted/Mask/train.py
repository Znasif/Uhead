...
def get_counter(dirpath):
    ...
    for dir in os.listdir(dirpath):
        for filename in os.listdir(join(dirpath, dir)):
            lens.append(1)
            letters += dir
    return Counter(letters)


c_val = get_counter(join(direc, "vald/"))
c_train = get_counter(join(direc, "train/"))
...
tiger_test = TextImageGenerator(join(direc, 'test'), 32, 28, 32, 4)
...
for inp_value, _ in tiger_test.all_data():
    ...
    for i in range(bs):
        ...
        print('Predicted: %s\nTrue: %s' % (pred_texts[i], int(texts[i])))
